...
Efter at have åbnet en indekseringsside med funktionen \methodref{open)}, var det nu nødvendigt med en metode til at finde de 16 opskriftslink. Problemet var bare at siden ikke kun indeholdt de 16 opskriftslink, men også links til nyheder, kontaktinformation og lignende. For at kunne filtrere i siden, så vi stod tilbage med de 16 ønskede opskriftslinks, benyttede vi Ruby-modulet \classref{Nokogiri}, der er en parser til blandt andet HTML. Med \classref{Nokogiri} var det let at filtrere en side efter bestemte tags og klasser. I vores tilfælde, havde vi brug for alle \textit{<a>} tags inde i \textit{<h2>} tags, som det kan ses af det givede linkeksempel. Dette blev gjort ved at benytte \classref{Nokogiri}'s funktion:

\texttt{xpath("//h2//a").map{|link| "http://www.arla.dk"+link["href"]}}

Kaldet til \methodref{xpath}, finder alle \textit{<a>} tags inde i \textit{<h2>} tags og returnerer et array af elementer, der hver især har en url til en opskrift for indekset \textit{href}. Ved at bruge funktionen \textit{map}, fås værdien af indekset \textit{href}, og præfixet \texttt{http://www.arla.dk} tilføjes foran.
